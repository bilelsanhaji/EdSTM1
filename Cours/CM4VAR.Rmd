...
\text{Équation} & y_t = \phi_0 + \phi_1 y_{t-1} + \varepsilon_t & \mathbf{y}_t = \boldsymbol{\Phi}_0 + \boldsymbol{\Phi}1 \mathbf{y}{t-1} + \boldsymbol{\varepsilon}_t \\
...
\text{Autocovariance } s & \gamma_s = \phi_1^s V[y_t] & \boldsymbol{\Gamma}_s = \boldsymbol{\Phi}1^s V[\mathbf{y}t] \\
...
# chunk à ne pas utiliser !!
# chunk qui correspond aux données des slides 01/02/2025
#start_date <- as.Date("1980-01-01")
#end_date <- as.Date("2019-12-31")

# 1. Télécharger les données FRED
#gdp_data <- fredr(series_id = "GDPC1", 
#                  observation_start = start_date, 
#                  observation_end = end_date, frequency = "q")
#unemp_data <- fredr(series_id = "UNRATE", 
...
var_model <- VAR(data_diff[, c("d_log_gdp", "d_unemp")],
                 p = 1, type = "const")
...
adf_values <- data.frame(
  ...
  `Statistique ADF` = c(adf_unrate$statistic, adf_dff$statistic, adf_gdpdef$statistic),
  `p-value` = c(adf_unrate$p.value, adf_dff$p.value, adf_gdpdef$p.value)
)
adf_values
```
